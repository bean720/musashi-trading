...
public:
   ...
   bool              FontSize(const int value) { return(m_edit.FontSize(value) && m_list.FontSize(value));    }
   //--- chart event handler
   virtual bool      OnEvent(const int id,const long &lparam,const double &dparam,const string &sparam);
   //--- fill
   bool              AddItem(const string item,const long value=0);
   int               ItemsCount(){return (m_list.ItemsCount());}
   //--- set up
   ...
   bool              SelectByStringFind(const string text);
   //--- data (read only)
...
//--- check height
   ...
   if(y2-y1<CONTROLS_COMBOBOX_MIN_HEIGHT){
      Print(
               HEAD_TO_STRING+
               (string)y2+", "+
               (string)y1
...
//| Select item (by text)                                            |
//+------------------------------------------------------------------+
bool CComboBox::SelectByStringFind(const string text)
  {
   if(!m_list.SelectByStringFind(text))
      return(false);
//--- call the handler
   return(OnChangeList());
  }
